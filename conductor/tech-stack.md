# Technology Stack

## Backend
- **Language:** Python
- **Framework:** FastAPI (High performance, modern async support)
- **Task Queue:** TaskIQ (Asynchronous task management for Photoshop automation)

## Frontend (GUI)
- **Framework:** React or similar (To be integrated with FastAPI backend)

## Data Storage
- **Database:** SQLite (Serverless, efficient relational storage for structured data and PSD layer references)

## Core Libraries
- **Excel Integration:** xlwings (For deep Excel integration and automation)
- **PSD Automation:** psd-tools2 (For advanced Photoshop scripting and layer manipulation)
- **Data Parsing & Matching:** fuzzywuzzy / difflib (For nuanced, fuzzy data parsing and matching beyond strict regex)
- **Data Processing:** pandas (For efficient data handling)
